...
def index(request):
    if request.user.is_superuser:
        return render(request, 'GRsystem/home2.html')
    else:
...
@user_passes_test(lambda u: u.is_superuser) #remove this condition
def counter(request):
    if request.user.is_superuser:    
        ...
        return render(request,"GRsystem/counter2.html",args)
    else:
        total=Complaint.objects.all().count()
        unsolved=Complaint.objects.all().exclude(status='1').count()
        solved=Complaint.objects.all().exclude(Q(status='3') | Q(status='2')).count()
...
def show_stats(request):
        ...
        dataset=Complaint.objects.values('Type_of_complaint').annotate(total=Count('status'),solved=Count('status', filter=Q(status='1')),
                  notsolved=Count('status', filter=Q(status='3')),inprogress=Count('status',filter=Q(status='2'))).order_by('Type_of_complaint')
        args={'total':total,'unsolved':unsolved,'solved':solved,'dataset':dataset,}
        return render(request,"GRsystem/seestats.html",args)
...
def dashboard(request):
    if request.user.is_superuser:    
        ...
        return render(request, 'GRsystem/dashboard2.html',context)
    else:
        if request.method == 'POST':
            p_form=ProfileUpdateForm(request.POST,instance=request.user)
            profile_update_form=UserProfileUpdateform(request.POST,instance=request.user.profile)
...
def change_password(request):
    if request.user.is_superuser: 
        if request.method == 'POST':
            ...
            form = PasswordChangeForm(request.user)
        return render(request, 'GRsystem/change_password2.html', {
            'form': form
        })
    else:
        if request.method == 'POST':
    ...
    if request.user.is_superuser:   
        if request.method == 'POST':
            ...
            if complaint_form.is_valid():
                ...
                print(mail)
                #send_mail('Complaint has been Received', 'Thank you for letting us know of your concern, Have a Cookie while we explore into this matter.Dont Reply to this mail', request.user.email, [mail],fail_silently=False)
                send_mail('Grievance Received on SGS', 'Thank you for letting us know of your concern. Your grievance has been registered successfully. We will get back to you as soon as possible.\nPlease do not reply to this e-mail', request.user.email, [mail],fail_silently=False)
                instance.save()
        ...
        return render(request,'Grsystem/comptotal2.html',context)
    else:
        if request.method == 'POST':
...
def list(request):
    if request.user.is_superuser:   
        ...
        return render(request,'Grsystem/Complaints2.html',args)
    else:
        c=Complaint.objects.filter(user=request.user).exclude(status='1')
        result=Complaint.objects.filter(user=request.user).exclude(Q(status='3') | Q(status='2'))
        #c=Complaint.objects.all()
...
def slist(request):
    if request.user.is_superuser:   
        ...
        #c=Complaint.objects.all()
        args={'result':result}
        return render(request,'Grsystem/solvedcomplaint2.html',args)
    else:
        result=Complaint.objects.filter(user=request.user).exclude(Q(status='3') | Q(status='2'))
        #c=Complaint.objects.all()
        args={'result':result}
...
def allcomplaints(request):
        ...
        if request.method=='POST':
                ...
                if forms.is_valid():
                        ...
                        print(m)
                        send_mail('Hi,Complaint has been Resolved ', 'Thanks for letting us know of your concern,  Hope we have solved your issue.  Dont Reply to this mail', m, [m],fail_silently=False)
                        obj.save()
...
def pdf_viewer(request):
    ...
    Receiver = Complaint.objects.filter(id=cid).values('Receiver')
    ...
    for val in Receiver:
            detail_receiver=("{}".format(val['Receiver']))
    for val in Type:
    ...
    p.drawString(30, 730,"To:")
    p.drawString(30, 710,detail_receiver)
    p.drawString(30, 680,detailtime)
    p.drawString(30, 660,detailsubject)
    p.drawString(30, 630,"Dear Sir/Madam,")
    p.drawString(30, 610,"Our SMC has a complaint of the type: {type}. Our complaint is as follows:".format(type= detailtype))
    t11=590
    for line in detail_string.split('\n'):
        #p.drawString(30, t11,line.strip())
        if len(line) > 100:
            wrap_text = textwrap.wrap(line, width=100)
            for text in wrap_text:
                p.drawString(30, t11, text.strip())
                t11-=20
        else:
            p.drawString(30, t11, line.strip())
            t11-=20
    p.drawString(30, t11-10,"Thank you")
...
def pdf_view(request):
    ...
    for val in Issuedate:
            detailtime=("Date: {}".format(val['Time']))
    #detail_string = u", ".join(("Desc={}".format(val['Description'])) for val in details) 
